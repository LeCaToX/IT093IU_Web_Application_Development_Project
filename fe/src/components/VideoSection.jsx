...
const VideoSection = ({ cid, name }) => {
    ...
    const getCurrentVideos = () => {
        ...
        if (filteredVideos.length === 1) return [filteredVideos[0]]; // Return single video, don't duplicate
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">{/* Left Navigation Button */}
                {filteredVideos.length > 1 && (
                    <button
                        ...
                        className={`absolute top-1/2 -translate-y-1/2 left-2 z-20 p-3 bg-black/60 hover:bg-black/80 rounded-full transition-all duration-300 ease-out cursor-pointer backdrop-blur-sm border border-white/10 ${isTransitioning
                ...
                <div className="overflow-hidden rounded-lg">
                    <div
                        className={`grid grid-cols-1 md:grid-cols-2 gap-6 min-h-[300px] transition-all duration-500 ease-out transform ${isTransitioning
                    ...
                    >
                        {currentVideos.map((video, index) => (
                            <div
                                ...
                                className={`transform transition-all duration-500 ease-out ${isTransitioning
                ...
                {filteredVideos.length > 1 && (
                    <button
                        onClick={goToNext}
                        disabled={isTransitioning}
                        className={`absolute top-1/2 -translate-y-1/2 right-2 z-20 p-3 bg-black/60 hover:bg-black/80 rounded-full transition-all duration-300 ease-out cursor-pointer backdrop-blur-sm border border-white/10 ${isTransitioning
                                ? 'opacity-30 cursor-not-allowed scale-95'
                                : 'opacity-90 hover:opacity-100 hover:scale-110 hover:shadow-lg hover:shadow-black/30'
                            }`}
                    >
                    ...
                    <div className="flex justify-center mt-8 space-x-4">
                        {Array.from({ length: totalPositions }, (_, index) => (
                            <button
                                ...
                                disabled={isTransitioning}
                                className={`relative w-4 h-4 rounded-full transition-all duration-400 ease-out cursor-pointer transform overflow-hidden ${currentIndex === index
